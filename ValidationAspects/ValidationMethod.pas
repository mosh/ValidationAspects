...
begin
  ...
  var messageAssign := new AssignmentStatement(new BeginLocalValue(messageValue), new DataValue(self.Message));

  methodIfBegin.Add(messageAssign);

  var newRuleViolation := new NewValue(Services.GetType('Validation.RuleViolation'), [new BeginLocalValue(messageValue),new DataValue(aMethod.Name)]);
